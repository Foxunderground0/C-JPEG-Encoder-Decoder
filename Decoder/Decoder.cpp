...
tuple<uint64_t, uint16_t, uint16_t> find_start_of_frame_info(std::array<unsigned char, IMG_data_len>& arr) {
	for (unsigned int i = 0; i < arr.size(); i++) {
		if (arr.at(i) == (0xffc0 >> 8) && arr.at(i + 1) == (0xffc0 & 0xff) && arr.at(i + 4) == (0x08)) {
			return { i + 2, arr.at(i + 5) << 8 | arr.at(i + 6), arr.at(i + 7) << 8 | arr.at(i + 8) }; // index immedeately after tag, Height, Width
		}
	}
	return { -1, -1 , -1 };
}
